...
| preferred_language | SMALLINT    | No       | 0=Turkish, 1=English. Captured from Accept-Language at registration; updatable via `PUT /api/auth/me/language`. Drives the language of server-generated content addressed to this user (currently badge/quest notifications) — never the ambient request culture, since notifications can be generated by a different actor or a background job. |
